...
var users *mongo.Collection = db.Users(db.Client, "users")
...
func Generate(email string, fName string, lName string, uid string) (signed string, refresh string, err error) {
	...
	token, err := jwt.NewWithClaims(jwt.SigningMethodES256, sig).SignedString([]byte(SECRET))
	if err != nil {
		return "", "", err
	}

	rfTok, err := jwt.NewWithClaims(jwt.SigningMethodES384, rfSig).SignedString([]byte(SECRET))
	if err != nil {
		log.Panic(err)
		return	
	}
